...
public class EmployeeMapper implements Mapper<Employee, co.com.foundation.morphia.entities.Employee> {
	...
	public co.com.foundation.morphia.entities.Employee marshall(Employee input) {
		try {
			...
			builder.id(input.getId() != null ? new ObjectId(input.getId()) : null).firstName(input.getFirstName())
					.lastName(input.getLastName()).identification(input.getIdentification())
			...
			if (input.getManagerId() != null && !Utils.isToReject.test(input.getManagerId())) {
				builder.manager((co.com.foundation.morphia.entities.Employee.builder()
						.id(new ObjectId(input.getManagerId())).build()));
			}
